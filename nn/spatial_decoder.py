...
class GMWA(nn.Module):
    def __init__(self, in_features, out_features, n_heads, dim_head, n_source_nodes, 
                 ...
                 dropout, mc_probabilistic, mc_dropout, device
    ):
        """
        ...
        self.gat_layer1 = GraphAttentionV2Layer(240, in_features, n_heads)
...
class GMWANet(nn.Module):
    ...
    def forward(self, x, y, batch_size, sub_hemo, sub_ele, teacher_forcing_ratio):
        ...
        # x_temporal_downsample = rearrange(self.temporal_embedding(xfm_graph).squeeze(-1), '(b p) m -> b p m', p=n_parcels) 
