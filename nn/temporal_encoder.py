...
class WaveletAttentionNet(nn.Module):
    def __init__(self, args):
        ...
        hidden_size = 1024
        self.temporal_embedding = ThreeLayerMLP(sum(dims[:]), hidden_size, 16).to(device)
        # self.temporal_embedding = ThreeLayerMLP(sum(dims[:]), hidden_size, 1).to(device)
    ...
    def forward(self, x_meg_hrf):
        ...
        return rearrange(self.temporal_embedding(x_attentions), '(b p) m d -> b p (m d)', p=n_parcels), alphas
