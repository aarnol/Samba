...
import numpy as np
from utils.hrf.utils import anatomical_parcel_names
...
def xy_scatterplot(x, y, x_std, color_list, title_color, plt_label, convert_ms=True): 
    ...
    anatomical_main_regions = ['Frontal', 'Temporal', 'Parietal', 'Occipital', 'Cingulate', 'Insular', 'Somatomotor']
    ...
    labels = ['Cingulate ', 'Insular', 'Somatomotor']
    for i, (color, label) in enumerate(zip(colors, labels)):
        plt.annotate(
        f"{label}",
        xy=(x_position, y_position - i * 0.11),  # Decreasing y position for each label
...
def hrf_histrogram(model):   
    ...
    plt.subplot(6, 2, 1) 
    xy_scatterplot(x=response_delay[:hem_length], y=model.args.parcels_name[:hem_length], x_std=response_delay_std[:hem_length], color_list=lh_color_list, title_color='red', plt_label='LH Response Delay')
    plt.subplot(6, 2, 2)
    xy_scatterplot(x=response_delay[hem_length:], y=model.args.parcels_name[hem_length:], x_std=response_delay_std[hem_length:], color_list=rh_color_list, title_color='blue', plt_label='RH Response Delay')
    
    plt.subplot(6, 2, 3)
    xy_scatterplot(x=undershoot_delay[:hem_length], y=model.args.parcels_name[:hem_length], x_std=undershoot_delay_std[:hem_length], color_list=lh_color_list, title_color='red', plt_label='LH Undershoot Delay')
    plt.subplot(6, 2, 4)
    xy_scatterplot(x=undershoot_delay[hem_length:], y=model.args.parcels_name[hem_length:], x_std=undershoot_delay_std[hem_length:], color_list=rh_color_list,  title_color='blue', plt_label='RH Undershoot Delay')
        
    plt.subplot(6, 2, 5)
    xy_scatterplot(x=response_dispersion[:hem_length], y=model.args.parcels_name[:hem_length], x_std=response_dispersion_std[:hem_length], color_list=lh_color_list, title_color='red', plt_label='LH Response Dispersion')
    plt.subplot(6, 2, 6)
    xy_scatterplot(x=response_dispersion[hem_length:], y=model.args.parcels_name[hem_length:], x_std=response_dispersion_std[hem_length:], color_list=rh_color_list, title_color='blue', plt_label='RH Response Dispersion')
        
    plt.subplot(6, 2, 7)
    xy_scatterplot(x=undershoot_dispersion[:hem_length], y=model.args.parcels_name[:hem_length], x_std=undershoot_dispersion_std[:hem_length], color_list=lh_color_list, title_color='red', plt_label='LH Undershoot Dispersion')
    plt.subplot(6, 2, 8)
    xy_scatterplot(x=undershoot_dispersion[hem_length:], y=model.args.parcels_name[hem_length:], x_std=undershoot_dispersion_std[hem_length:], color_list=rh_color_list, title_color='blue', plt_label='RH Undershoot Dispersion')
        
    plt.subplot(6, 2, 9)
    xy_scatterplot(x=response_scale[:hem_length], y=model.args.parcels_name[:hem_length], x_std=response_scale_std[:hem_length], color_list=lh_color_list, title_color='red', plt_label='LH Response Scatter', convert_ms=False)
    plt.subplot(6, 2, 10)
    xy_scatterplot(x=response_scale[hem_length:], y=model.args.parcels_name[hem_length:], x_std=response_scale_std[hem_length:], color_list=rh_color_list, title_color='blue', plt_label='RH Response Scatter', convert_ms=False)
        
    plt.subplot(6, 2, 11)
    xy_scatterplot(x=undershoot_scale[:hem_length], y=model.args.parcels_name[:hem_length], x_std=undershoot_scale_std[:hem_length], color_list=lh_color_list, title_color='red', plt_label='LH Undershoot Scatter', convert_ms=False)
    plt.subplot(6, 2, 12)
    xy_scatterplot(x=undershoot_scale[hem_length:], y=model.args.parcels_name[hem_length:], x_std=undershoot_scale_std[hem_length:], color_list=rh_color_list, title_color='blue', plt_label='RH Undershoot Scatter',  convert_ms=False)
