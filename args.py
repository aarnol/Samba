...
def parcel_extractor():
    ...
    vertex_200_labels, parcels200_name, ctabs = SchaeferParcel_Kong2022_17Network(parcel_number=200)
    # parcels200_name = [name[13:-1] for name in names]  # Adjusting name slicing for specific format
    # parcel200_name_vertex = vertex_200_labels
    parcels200_name.pop(0)  # Remove the background entry 
    parcels200_name.pop(100)
    
    
    vertex_500_labels, names_500, ctabs_500 = SchaeferParcel_Kong2022_17Network(parcel_number=500)
...
def params_fn(server_mode='misha', dataset='eegfmri_translation'):
    ...
    parser.add_argument("--mc_probabilistic", default=True) 
    ...
    params.hemo_adjacency_matrix_dir = params.graph_dir + 'arman/fmri-'+str(params.n_hemo_parcels)+'parcels/'
    if params.dataset=='megfmri':   
        params.ele_adjacency_matrix_dir = params.graph_dir + 'arman/meg-'+str(params.n_ele_parcels)+'parcels/'   
    elif params.dataset=='eegfmri_translation':   
        params.ele_adjacency_matrix_dir = params.graph_dir  + 'arman/eeg-'+str(params.n_ele_parcels)+'parcels/'
